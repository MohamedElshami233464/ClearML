...
class ClearMLLoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.locals.get("rollout_buffer", None):
            ep_len_mean = self.locals["rollout_buffer"].ep_len_mean
            ep_rew_mean = self.locals["rollout_buffer"].ep_rew_mean
...
# Add WandB and ClearML callbacks
...
model.learn(
    ...
    callback=[wandb_callback, clearml_callback],
    progress_bar=True,
