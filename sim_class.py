...
class Simulation:
    ...
    def log_joint_info(self, robotId):
        num_joints = p.getNumJoints(robotId)
        print(f"Number of joints in robot {robotId}: {num_joints}")
        for joint_index in range(num_joints):
            joint_info = p.getJointInfo(robotId, joint_index)
            print(f"Joint {joint_index}: Name={joint_info[1].decode('utf-8')}, Type={joint_info[2]}, Parent Link Index={joint_info[16]}")
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    self.log_joint_info(robotId)  # Add this line
    ...
    def get_pipette_position(self, robotId):
        # Ensure the robotId exists in self.robotIds
        if robotId not in self.robotIds:
            raise ValueError(f"Invalid robotId: {robotId}. Available IDs: {self.robotIds}")

        # Get the position of the robot
        robot_position = p.getBasePositionAndOrientation(robotId)[0]
        robot_position = list(robot_position)

        # Use the correct joint indices (Slider_3, Slider_4, Slider_5)
        joint_states = p.getJointStates(robotId, [0, 1, 2])

        # Adjust the robot position based on joint states
        robot_position[0] -= joint_states[0][0]  # X-axis adjustment
        robot_position[1] -= joint_states[1][0]  # Y-axis adjustment
        robot_position[2] += joint_states[2][0]  # Z-axis adjustment

        # Add offsets for the pipette position
